...
{ 
  ...
  //  theProblemList = new ArrayList<problems>();
   ...
   tableData = getAllMotors();
    theProblemList = getAllProblems();
...
//maybe have to add another dialog window to grab info to add
        	...
        	temp = table.getModel().getValueAt(o, 0);

                System.out.println(temp);
                

            
                            EditMotorForm dialog = new EditMotorForm(new javax.swing.JFrame(), true, temp);
                dialog.addWindowListener(new java.awt.event.WindowAdapter() {
                    @Override
                    public void windowClosing(java.awt.event.WindowEvent e) {
                        dialog.dispose();
...
displayTableData(getAllMotors(),2);
